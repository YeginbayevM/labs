...
@check50.check()
def exists():
    """helpers.c существует"""
    check50.exists("helpers.c")
    check50.include("Makefile", "bmp.h", "helpers.h", "testing.c")

@check50.check(exists)
def compiles():
    """filter компилируется"""
    check50.run("make").exit(0)

@check50.check(compiles)
def grayscale_single_pixel():
    """фильтр оттенки серого (grayscale) корректно применяется на одном пикселе со средним значением целого числа"""
...
def grayscale_rounding():
    """фильтр оттенки серого (grayscale) корректно применяется на одном пикселе без среднего значения целого числа"""
...
@check50.check(compiles)
def grayscale_gray():
    """фильтр оттенки серого (grayscale) не затрагивает пиксели, что уже являются серыми"""
...
@check50.check(compiles)
def grayscale_multi():
    """фильтр оттенки серого (grayscale) корректно применяется на простом изображении 3x3"""
    log(SAMPLE_IMAGES[0])
    check50.run("./testing 0 3").stdout("85 85 85\n" * 9);

@check50.check(compiles)
def grayscale3x3():
    """фильтр оттенки серого (grayscale) корректно применяется на более сложных изображениях 3x3"""
    log(SAMPLE_IMAGES[1])
...
@check50.check(compiles)
def grayscale4x4():
    """фильтр оттенки серого (grayscale) корректно применяется на изображении 4x4"""
    log(SAMPLE_IMAGES[2])
...
def sepia_single_pixel():
    """фильтр сепия (sepia) корректно применяется на одном пикселе"""
    log(SAMPLE_PIXEL(20, 40, 90))
...
@check50.check(compiles)
def sepia_multi():
    """фильтр сепия (sepia) корректно применяется корректно применяется на простом изображении 3x3"""
    log(SAMPLE_IMAGES[0])
...
@check50.check(compiles)
def sepia3():
    """фильтр сепия (sepia) корректно применяется на более сложных изображениях 3x3"""
    log(SAMPLE_IMAGES[1])
...
@check50.check(compiles)
def sepia4():
    """фильтр сепия (sepia) корректно применяется на изображениях 4x4"""
    log(SAMPLE_IMAGES[2])
...
@check50.check(compiles)
def reflect_row2():
    """изображение 1x2 корректно отражается с помощью фильтра отражение (reflect)"""
    log(SAMPLE_IMAGES[3])
...
@check50.check(compiles)
def reflect_row3():
    """изображение 1x3 корректно отражается с помощью фильтра отражение (reflect)"""
    log(SAMPLE_IMAGES[4])
...
@check50.check(compiles)
def reflect_simple():
    """фильтр отражение (reflect) корректно применяется на уже отражённое изображение"""
    log(SAMPLE_IMAGES[0])
...
@check50.check(compiles)
def reflect3():
    """фильтр отражение (reflect) корректно применяется на изображение 3x3"""
    log(SAMPLE_IMAGES[1])
...
@check50.check(compiles)
def reflect4():
    """фильтр отражение (reflect) корректно применяется на изображение 4x4"""
    log(SAMPLE_IMAGES[2])
...
@check50.check(compiles)
def blur_middle():
    """фильтр размытие (blur) корректно применяется на средний пиксель"""
    log(SAMPLE_IMAGES[1])
    check50.run("./testing 3 0").stdout("127 140 149\n")

@check50.check(compiles)
def blur_edge():
    """фильтр размытие (blur) корректно применяется на крайний пиксель"""
    log(SAMPLE_IMAGES[1])
    check50.run("./testing 3 1").stdout("80 95 105\n")

@check50.check(compiles)
def blur_corner():
    """фильтр размытие (blur) корректно применяется на пиксель в углу"""
    log(SAMPLE_IMAGES[1])
    check50.run("./testing 3 2").stdout("70 85 95\n")

@check50.check(compiles)
def blur3():
    """фильтр размытие (blur) корректно применяется на изображение 3x3"""
    log(SAMPLE_IMAGES[1])
...
@check50.check(compiles)
def blur4():
    """фильтр размытие (blur) корректно применяется на изображение 4x4"""
    log(SAMPLE_IMAGES[2])
